function Rfullstack = reflectivitystack(ng,kg,per)
...
%n1 =  (2.6 - 1.3i) .* ones(size(x));%ref index of graphene/graphite
n1 =  (ng + (kg)*1i) .* ones(size(x));
n2 = 1.8 - (0.00069375 .* (lambda - 480));%ref index of hBN with 3% dispersion
%n2 = (1.8).* ones(size(x)); %ref index of hbn
...
n4 = (1+per)*yyr + 1i.*(yyi); %ref index of silicon
...
d2 = 0.023; %size of hbn layer in micron
%l*0.0004; 
